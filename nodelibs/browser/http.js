import { a as dew$l, d as dew$m } from './chunk-bb904940.js';
...
function dew$k() {
  ...
  exports$l.abortController = isFunction(_global$5.AbortController); // The xhr request to example.com may violate some restrictive CSP configurations,
  ...
  function getXHR() {
    ...
    if (_global$5.XMLHttpRequest) {
      xhr = new _global$5.XMLHttpRequest(); // If XDomainRequest is available (ie only, where xhr might not work
  ...
  exports$l.arraybuffer = exports$l.fetch || checkTypeSupport("arraybuffer"); // These next two tests unavoidably show warnings in Chrome. Since fetch will always
  // be used if it's available, just return false for these to avoid the warnings.

  exports$l.msstream = !exports$l.fetch && checkTypeSupport("ms-stream");
  exports$l.mozchunkedarraybuffer = !exports$l.fetch && checkTypeSupport("moz-chunked-arraybuffer"); // If fetch is supported, then overrideMimeType will be supported too. Skip calling
  // getXHR().
...
function dew$e() {
  ...
  function Writable(options) {
    Duplex = Duplex || dew$d(); // Writable ctor is applied to Duplexes, too.
